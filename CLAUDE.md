...
#### 展開ルール
C1〜C3の条件では、推薦セクションを最初から展開状態で表示：
- **C1（色相推薦のみ）**: 色相推薦セクションを展開
- **C2（トーン推薦のみ）**: トーン推薦セクションを展開
- **C3（二段階推薦）**: 色相推薦・トーン推薦の両方を展開
- **C0（推薦なし）**: 推薦セクション自体が非表示のため対象外

#### 実装方法
- **初期化タイミング**: 条件（condition）が設定されたとき
- **useEffect依存配列**: `[condition]` で条件変更を監視
- **コラプス状態更新**:
  ```typescript
  useEffect(() => {
...
#### 目的
- **即座に利用可能**: 実験参加者が推薦機能をすぐに確認・利用できる
- **操作負担軽減**: 手動で展開する手間を削減
- **スムーズな実験体験**: ストレスの少ない実験環境を提供
