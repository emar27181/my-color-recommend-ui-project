...
const CanvasColorRecommendationsComponent = forwardRef<CanvasColorRecommendationsRef, CanvasColorRecommendationsProps>(({ className = '' }, ref) => {
  ...
  const floodFill = useCallback((startX: number, startY: number, newColor: string) => {
    ...
    // スタックベースのフラッドフィル（軽量化版）
    const filledPixels = new Set<number>(); // 数値インデックス管理（軽量化）
    const expandedPixels = new Set<number>(); // 拡張領域（軽量化）
    ...
    while (stack.length > 0) {
      ...
      if (shouldFill(x, y)) {
        ...
        filledPixels.add(index);
        // 隣接する4方向をスタックに追加（境界チェック付き）
        if (x + 1 < width) stack.push(index + 1);           // 右
        if (x - 1 >= 0) stack.push(index - 1);              // 左
    ...
    // === 軽量化されたピクセル塗りつぶし ===
    let paintedCount = 0;
    // 基本領域を塗りつぶし（数値インデックス直接使用）
    for (const pixelIndex of filledPixels) {
      const layerIndex = pixelIndex * 4;
  ...
  return (
    <Card className={`w-full flex flex-col bg-background border-transparent ${className}`} style={{ height: 'auto' }}>
      ...
      <CardContent className="pt-1 pb-0 flex-shrink-0">
...
CanvasColorRecommendationsComponent.displayName = 'CanvasColorRecommendations';

export const CanvasColorRecommendations = CanvasColorRecommendationsComponent;