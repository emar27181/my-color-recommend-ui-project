    ...
    <div className="w-full border rounded-xl bg-card p-6" style={{ 
      ...
      <div className="hidden lg:block mb-8">
        ...
        <div className="flex flex-wrap gap-4 p-4 bg-muted/30 rounded-lg">
          {testColors.map((item, index) => (
            <div key={index} className="flex items-center gap-3 bg-card border border-border rounded-lg p-3 shadow-sm">
              <div 
              ...
              <button
                ...
                className="p-2 rounded-full bg-muted hover:bg-muted/80 transition-colors opacity-80 hover:opacity-100"
                title="色をコピー"
              >
      ...
      <div className="hidden md:block lg:hidden mb-8">
        ...
        <div className="space-y-4">
          {[0, 1].map((rowIndex) => (
            <div key={rowIndex} className="flex gap-4 p-4 bg-muted/30 rounded-lg">
              {testColors.slice(rowIndex * 4, (rowIndex + 1) * 4).map((item, index) => (
                <div key={index} className="flex items-center gap-3 bg-card border border-border rounded-lg p-3 shadow-sm flex-1">
                  <div 
                    className="border-2 border-gray-300 rounded cursor-pointer hover:scale-110 transition-transform"
                    style={{ 
                      backgroundColor: item.color,
      ...
      <div className="block md:hidden">
        <h3 className="text-base font-medium mb-4 text-foreground">スマホ版: 縦並びレイアウト</h3>
      ...
      <div className="mt-8 p-4 bg-muted/20 rounded-lg">
        ...
        <div className="text-sm text-muted-foreground space-y-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <span>スマホ (〜767px): 縦並び、スクロール可能</span>
          </div>
        </div>
      </div>
    </div>
