...
### PC版色相環選択バーの表示列数問題
- **問題**: PC版の色相環選択バー（ColorRecommendations内のドロップダウン）が2列表示のまま
- **要望**: PC版で4列表示に変更、色相環コンポーネントサイズは維持
- **調査済み原因**:
  - App.tsxでColorRecommendationsが`grid-cols-2`の親コンテナ内に配置され幅制限
  - ドロップダウンが`left-0 right-0`で親要素幅に制限される
  - グリッドクラス`md:grid-cols-4 lg:grid-cols-5`は正しく設定済み
- **試行済み解決策**:
  - ドロップダウン幅の固定設定（`width: '500px'`）
  - 最大高さの調整（`max-h-60` → `max-h-80`）
  - レスポンシブブレークポイントの調整
- **次回対応方針**: 
  - ドロップダウンコンテナのポジショニング戦略見直し
  - 親コンテナ制約の回避方法検討
  - Portal使用による独立表示の検討

### 色相環の点の色反転問題
- **問題**: ダークモード時に色相環の点（プロット点）が適切に色反転されない
- **影響範囲**: ColorWheel.tsx, ColorWheelMini.tsx両コンポーネント
- **現状**: 固定色（青：`#3b82f6`、紫：`#8b5cf6`、白境界線：`#ffffff`）を使用
- **問題点**: 固定色のため、ダークモード背景で視認性が低下する可能性
- **要望**: ライト・ダークモード両方で適切にコントラストが確保された色表示
- **試行済み解決策**:
  - CSS変数からの固定色への変更（`--primary`, `--accent` → 固定HEX値）
  - 白色境界線の追加
- **次回対応方針**:
  - CSS変数の適切な活用（テーマ対応色の使用）
  - ダークモード専用の色指定検討
  - SVGのCSS classベースの色制御への変更検討
