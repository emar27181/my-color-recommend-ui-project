...
### UIテストページ折りたたみボタンの色を修正

**変更内容:**
- ChevronUp/ChevronDownアイコンの色を `text-foreground` から `text-muted-foreground` に変更
- ダークモードでも適切な控えめな色で表示されるように修正

**理由:**
- ダークモードで白色が目立ちすぎていたため
- より控えめな色でUIの統一感を向上
...
### UIテストページのコントロールパネルを折りたたみ可能に変更

**変更内容:**
- コントロールパネル（UI条件・タスク選択）を折りたたみ可能に変更
- CardHeaderに ChevronUp/ChevronDown ボタンを追加
- 折りたたみ状態を useState で管理
- 折りたたみ時は CardDescription と CardContent を非表示

**理由:**
- 画面スペースの有効活用
- 設定完了後はコントロールパネルを隠して作業領域を拡大
...
**変更内容:**
- 従来の4問+4問構成から、8問統一構成に変更
- 質問内容を更新：
  1. 必要な色の方向性がつかみやすく、目的の色に到達しやすかった
  2. 色選びで迷うことが少なかった
  3. 候補の色同士の違いがわかりやすかった
  4. イメージ（テーマ）に合う色が選びやすかった
  5. 最終的に選んだ色に納得できた
  6. 色の候補が多すぎると感じた（逆転項目）
  7. UIの色の提示方法（表示方法や手順）は色選びの役に立った
  8. 選んだ色に自信を持つことができた
...
### Test1・Test2でベースカラー選択を非表示に変更

**変更内容:**
- Test1（カラーパレット）とTest2（色相環+スライダー）では、ベースカラー選択セクションを非表示
- Test3（推薦機能あり）では従来通りベースカラー選択を表示

**理由:**
- T1/T2では直接色を選択するUIのため、ベースカラー選択が不要
...
**コミット:** `12c47cb` - feat: Test1・Test2でベースカラー選択を非表示に変更
